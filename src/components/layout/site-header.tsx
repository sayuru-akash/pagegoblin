...
export function SiteHeader() {
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border bg-cave/90 backdrop-blur-xl">
      <div className="mx-auto flex h-[72px] max-w-[1320px] items-center justify-between px-5 sm:px-8">
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/signin"
            className="hidden px-3 py-2 text-sm text-muted transition-colors hover:text-ink focus-goblin sm:inline-flex"
          >
            Sign in
          </Link>
          <Link
          ...
          <details className="relative lg:hidden">
            ...
            <nav className="absolute right-0 top-12 flex min-w-52 flex-col border border-border bg-cave p-2 shadow-2xl" aria-label="Mobile navigation">
              ...
              <Link href="/signin" className="px-4 py-3 text-sm text-muted transition-colors hover:bg-bone hover:text-ink focus-goblin">
                Sign in
              </Link>
